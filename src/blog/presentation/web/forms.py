"""
Состояние формы поста и то, как оно превращается в страницу.

Отдельно от pages.py, потому что это единственное место на всём сайте,
где страница зависит не только от базы, но и от того, что человек только
что набрал и чем это не устроило валидатор. В роутах остаётся два шага:
получить состояние и отрисовать его.
"""
